...
from தன்விவரங்கள்.வளங்கள் import பொது_மொழிபெயர்ப்புகளைப்_பேறு
    ...
    def __call__(தன், சாபி: str, மொழி: str) -> Optional[str]:
        ...
        for இ, சாபி_பகுதி in enumerate(சாபி_பகுதிகள்):
            try:
                ...
                if இ == (len(சாபி_பகுதிகள்) - 1) and அகராதி:
                    return அகராதி
            except KeyError:
                break

        முன்னிருப்பு = பொது_மொழிபெயர்ப்புகளைப்_பேறு(மொழி)
        if முன்னிருப்பு:
            அகராதி = முன்னிருப்பு
            for இ, சாபி_பகுதி in enumerate(சாபி_பகுதிகள்):
                try:
